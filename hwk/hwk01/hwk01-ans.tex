...
% \(x \in \mathcal{S}/\emptyset\) such that \(x \not \in A\) and \(x \not \in B\). By definition, we have \(x \in A^c\) and \(x \in B^c\), so 
% \(x \in A^c \cap B^c\), which means that \(A^c\) and \(B^c\) are not disjoint. For \(A^c\) and \(B^c\) to be disjoint, we require 
% \(A^c \cap B^c = \emptyset\). Using DeMorgan's Laws, we have \((A \cup B)^c = \emptyset\)
